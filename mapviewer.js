  ...
  componentDidMount() {
    ...
    setInterval(function() {
      map.eachLayer(function(layer) {
        if (
          layer.feature &&
          new Date() - layer.feature.properties.created > 15 * 60 * 1000
        ) {
    ...
    var createIslandLabel = function(island) {
      ...
      label +=
        '<div class="islandlabel_icon"><img class="islandlabel_size" src="' +
      ...
      .then(function(islands) {
        for (let k in islands) {
          if (islands[k].animals || islands[k].resources) {
            var circle = new IslandCircle(
              unrealToLeaflet(islands[k].worldX, islands[k].worldY),
              {
                radius: 1.5,
                color: "#f00",
                opacity: 0,
